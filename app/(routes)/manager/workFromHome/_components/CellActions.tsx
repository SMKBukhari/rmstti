...
const CellActions = ({ user, id }: CellActionsProps) => {
  ...
  const onSubmit = async () => {
    try {
      setIsLoading(true);
      await axios.delete(
        `/api/user/${user?.userId}/workFromHomeRequests/${id}`
      );
      toast.success(`Work From Home request Deleted successfully.`);
      setDialogOpen(false);
      setIsLoading(false);
      router.refresh();
    } catch (error: unknown) {
  ...
  return (
    <div className='flex gap-3' onClick={handleButtonClick}>
      ...
      <DialogForm
        ...
        description='Are you sure you want to delete this Work From Home request?'
        fields={[
          {
            name: "agree",
            type: "input",
