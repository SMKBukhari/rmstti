...
import type { Department, JobApplications, Role, Status, UserProfile } from "@prisma/client"
...
interface UserAboutSectionProps {
  ...
  applicant: (UserProfile & { role: Role | null } & { status: Status | null }) | null
  userJobApplications: (UserProfile & { jobApplications: JobApplications[] }) | null
  department: Department[] | null
  role: Role[] | null
}

const UserAboutSection = ({ user, applicant, userJobApplications, department, role }: UserAboutSectionProps) => {
  const [isDialogOpen, setDialogOpen] = useState(false)
  const [isSecondDialogOpen, setSecondDialogOpen] = useState(false)
  const [isLoading, setIsLoading] = useState(false)
  const [isRejection, setIsRejection] = useState(false)
  const [currentJobApplication, setCurrentJobApplication] = useState<JobApplications | null>(null)
  ...
  useEffect(() => {
    if (applicant?.currentJobApplicationId && userJobApplications) {
      const currentApp = userJobApplications.jobApplications.find((app) => app.id === applicant.currentJobApplicationId)
      setCurrentJobApplication(currentApp || null)
    }
  }, [applicant, userJobApplications])

  const isOffered = currentJobApplication?.applicationStatusId === "cm5cbm4m70004z0fcty2jear6" // Offered
  ...
  const avatarFallback = applicant?.fullName?.substring(0, 2).toUpperCase() || "U"
  return (
    <main>
      <div className="w-full flex flex-col gap-10 px-5 py-10 bg-[#FFFFFF] dark:bg-[#0A0A0A] rounded-xl mt-5">
        ...
        {userJobApplications?.jobApplications.map((application, index) => (
          <div key={index} className="flex flex-col gap-1 -mt-6">
            {application.reference ? (
              <h3 className="text-muted-foreground text-base">{application.reference}</h3>
            ) : (
              <h3 className="text-muted-foreground text-base">No reference provided.</h3>
            )}
            {application.referenceContact ? (
              <h3 className="text-muted-foreground text-base">{application.referenceContact}</h3>
            ) : (
              <h3 className="text-muted-foreground text-base">No reference contact provided.</h3>
            )}
          </div>
        ))}
        ...
        <div className="flex sm:flex-row flex-col flex-wrap md:justify-end justify-center gap-4">
          <Button variant={"primary"} onClick={() => setDialogOpen(true)}>
            {currentJobApplication?.isInterviewed ? "Update Interview" : "Interview"}
          </Button>
          <Button variant={"destructive"} onClick={onReject}>
            {isRejection ? <Loader2 className="w-4 h-4 animate-spin" /> : <span>Reject</span>}
          </Button>
        </div>
        <Button variant={"success"} onClick={() => setSecondDialogOpen(true)} disabled={isOffered || isHired}>
          Hire Applicant
        </Button>
      </div>
      ...
      <DialogForm
        ...
        fields={[
          ...
          },
          {
            ...
            label: "Department",
            type: "select",
            comboboxOptions: department ? department.map((d) => ({ label: d.name, value: d.name })) : [],
          },
          {
            name: "role",
            label: "Role",
            type: "select",
            comboboxOptions: role ? role.map((r) => ({ label: r.name, value: r.name })) : [],
          },
          {
            name: "salary",
            label: "Salary",
