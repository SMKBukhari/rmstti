...
export const POST = async (req: Request) => {
  try {
    ...
    const uploadResponse = await new Promise<cloudinary.UploadApiResponse>(
      (resolve, reject) => {
        const stream = cloudinary.v2.uploader.upload_stream(
          {
            folder: "offerLetter", // Specify folder
            resource_type: "raw", // Specify that it's not an image
            public_id: `${user?.fullName}.pdf`, // Use userId as public ID
          },
          (error, result) => {
            if (error) {
              reject(error);
    ...
    const {
      secure_url: offerLetterUrl,
      public_id: offerLetterPublicId,
      original_filename: offerLetterName,
    } = uploadResponse;
    ...
    const updatedUser = await db.userProfile.update({
      ...
      data: {
        ...
        offerLetterName: `${user?.fullName} Offer Letter ${offerLetterName}.pdf`,
        offerLetterPublicId,
      },
    });
